...
struct Config {
    ...
    static let leftWaveY = 100.0 //CHANGE
...
struct SliderData {
    ...
    init(side: SliderSide, startPosition: Double) {
        self.side = side
        self.centerY = startPosition
        self.progress = 0
    ...
    func initial(position: Double) -> SliderData {
        return SliderData(side: side, centerY: position, progress: 0)//change centerY to center of image
    ...
    func drag(value: DragGesture.Value) -> SliderData {
        ...
        //let progress = max(0, dx * Config.swipeVelocity / SliderData.width)
